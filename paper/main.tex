...
% \title{Towards Better Understanding Table Instruction Tuning: Decoupling the Effects from Data versus Models}
% \title{Positioning Table Understanding: Paradigm Shifts, Present Challenges, and What Lies Ahead}
...
$^{2}$\texttt{zshe@amazon.com}; $^{3}$\texttt{hzhu@figma.com} \\
}
...
% \dnaihao{Better models in general;
% However, now we are faced with many choices in terms of the model selection, data selection, etc.}
% \dnaihao{Moreover, there is no systematic comparison in the existing works.}
% \dnaihao{Three key questions: 1) Would there be a difference when we choose different base models? If so, how large would the difference be?;
% 2) What is the best training data that can make the model generalize well?}
...
\input{figures/in-domain-avg}
\input{tables/out-of-domain-complete}
...
\begin{table*}[t]
    ...
    \bottomrule
    \end{tabular}
    \caption{Types of reasoning errors commonly made by tableLLMs, with their description and example erroneous responses (\includegraphics[width=10pt]{imgs/icons/robot3.pdf}) to questions (\includegraphics[width=9pt]{imgs/icons/question.pdf}) from our experiment results on the Phi model trained on TableLLM data.}
...
% \dnaihao{To enhance generalization, future work could explore multi-task training strategies, curriculum learning, or synthetic data augmentation. Given our findings, incorporating explicit reasoning traces during training might particularly benefit generalization.}
...
% \dnaihao{Maybe another plot here to show the correlations?}
...
\input{figures/general-benchmark-performance-difference}
